...
test(
  'noirforge devnet e2e (triggered): build/prove/deploy/verify-onchain (sum_a_b)',
  {
    timeout: 25 * 60 * 1000,
    skip: process.env.NOIRFORGE_E2E_DEVNET !== '1',
  },
  async () => {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), 'noirforge-e2e-devnet-'));
    const circuitDir = path.join(tmp, 'sum_a_b');
    const artifactName = `e2e_sum_a_b_${Date.now()}`;
